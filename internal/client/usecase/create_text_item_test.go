...
type testCreateTextServer struct {
	pb.UnimplementedGophKeeperServiceServer
}

func (s *testCreateTextServer) CreateItem(ctx context.Context, req *pb.CreateItemRequest) (*pb.CreateItemResponse, error) {
	id := "created-item-1"
	title := req.GetTitle()
	meta := req.GetMeta()
...
func startCreateTextTestServer(t *testing.T, srv pb.GophKeeperServiceServer) (string, func()) {
	t.Helper()
...
func TestCreateTextItemUseCase_Integration(t *testing.T) {
	addr, cleanup := startCreateTextTestServer(t, &testCreateTextServer{})
	defer cleanup()
	...
	uc := NewCreateTextItemUseCase(store, client)

	itemID, err := uc.Execute(context.Background(), "note1", "hello world", "meta1", "master-password")
	if err != nil {
		t.Fatalf("Execute returned error: %v", err)
	}

	if itemID != "created-item-1" {
		t.Fatalf("expected created-item-1, got %q", itemID)
	}
